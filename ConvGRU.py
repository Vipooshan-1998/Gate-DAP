...
class ConvGRU(nn.Module):
    def __init__(self, input_size, input_dim, hidden_dim, kernel_size, num_layers,
                 ...
                 long_gate=True, short_gate=False, use_trace=False):
        """
        :param input_size: (int, int)
            Height and width of input tensor as (height, width).
        ...
        '''conv_gate'''
        self.gate_conv_h = Gated_Conv(in_ch=input_dim,out_ch=input_dim)
        self.gate_conv_x = Gated_Conv(in_ch=input_dim,out_ch=input_dim)
    ...
    def forward(self, input_tensor, hidden_state=None):
        ...
        for layer_idx in range(self.num_layers):
            ...
            for t in range(seq_len):
                ...
                # if self.use_trace and t>0:
                #     avg_trace = torch.mean(torch.mul(cur_layer_input[:, t, :, :, :], cur_layer_input[:, t-1, :, :, :]))
                #     input_x = cur_layer_input[:, t, :, :, :] + avg_trace*cur_layer_input[:, t, :, :, :]
                # else:
                #     input_x = cur_layer_input[:, t, :, :, :]

                if self.short_gate:
                    input_x = self.gate_conv_x(input_x)
                else:
                    input_x = cur_layer_input[:, t, :, :, :]

                h = self.cell_list[layer_idx](input_tensor=input_x, # (b,t,c,h,w)
                                              h_cur=h)
                if self.long_gate:
                    h = self.gate_conv_h(h)
